...
ビルダーは **`90_attachments\<citekey>\infographic.png` があれば `# 1 AI要約` の直下に埋め込む。**
インフォグラフィックはユーザーが別途生成して置いているので、**ビルダー側は消さない・作らない・参照するだけ。**
2026-09-15まで参照しておらず、そのせいで画像付きのノートが別系統で作られ続けていた（LESSONS 27）。
...
> この手順は、実際の作業の型（既存確認 → PubMed照合 → PDF/本文 → Zotero → ノート・概念ノート → 検証 → 報告）を
> スキル **`paper-import`**（`C:\Users\user\claude\.claude\skills\paper-import\`）にまとめてある。
> 論文を取り込むときはそれに従う。**食い違いがあれば、この節（CLAUDE.md）を優先する。**
...
手で書いた未取得リストは必ず古くなり、**取得済みの論文をユーザーに依頼する**事故につながる。
実体（`fulltext\<id>.pdf` があるか）を毎回見ること。

```bash
pwsh -File C:\Users\user\claude\papers\pdf_status.ps1
```

`library.json` を走査して4つに分ける — **私が取得できる（PMC掲載）／ユーザーにお願いする（購読誌）／
入手困難（依頼しない）／取得済み。** 結果は画面と `vault\未取得PDF.md` の両方に出る。

入手困難と分かったら `library.json` のエントリに **`pdf_status: "unobtainable"`** と
**`pdf_note`（理由と、必要になったときの入手手段）** を書く。以後そのリストに落ちるので、
**同じ論文を繰り返し依頼しなくなる。**

PDFを依頼する前に必ずこれを走らせること。
