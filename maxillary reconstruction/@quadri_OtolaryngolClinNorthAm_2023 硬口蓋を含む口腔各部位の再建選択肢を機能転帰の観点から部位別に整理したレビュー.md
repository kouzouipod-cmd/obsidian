---
tags:
  - PMID/37164898
citekey: "quadri_OtolaryngolClinNorthAm_2023"
dateread: '2026-09-15'
read: false
topic: maxillary_reconstruction
domain: head_neck
relevance: medium
arm: review-overview
---
> [!Data]
> **PDF**
> （全文なし）
> **Link**
> https://doi.org/10.1016/j.otc.2023.04.002

# 1 AI要約
![[90_attachments/quadri_OtolaryngolClinNorthAm_2023/infographic.png]]


## AI要約
### **研究の概要**
- 口唇・舌・口底・頬粘膜・硬口蓋・複合的な口腔下顎切除に至る口腔各部位の再建を、機能転帰の最適化という観点から整理したレビュー。

### **先行研究との差異および優位性**
- 一次縫縮・二次治癒・植皮・真皮代替から局所・領域・遊離皮弁までを、部位ごとに適応の階層として示す。

### **研究手法**
- ナラティブレビュー。

### **結果**
- 部位別（口唇・舌・口底・頬・硬口蓋・複合欠損）の軟部および骨再建の選択肢を提示。
- 一次縫縮、二次治癒、植皮、真皮代替材の適切な適応を明示。
- 局所・領域・遊離皮弁それぞれの解剖学的考慮点、適応、禁忌、合併症を論じる。
- ⚠️ レビューのため症例数・統計値なし。

### **考察および課題**
- **「シングル軟部」群の下限を定義するのに有用。** 硬口蓋の小欠損では皮弁自体が不要な場合があることを示す。
- 本研究の対象を「どこから遊離皮弁が必要か」で線引きする際の根拠になる。
- 主眼は口腔であり上顎そのものではないため、補助的な位置づけ。

### **キーワード**
- #oral_cavity_reconstruction, #hard_palate, #reconstructive_ladder, #free_flap, #review

### **wikilink**
- [[Maxillary Reconstruction]]
- [[Head and Neck Reconstruction]]
- [[Free Flap]]

# 2 Citation
Quadri P, McMullen C. Oral Cavity Reconstruction. Otolaryngol Clin North Am. 2023;56(4):671-686.

# 3 Related
>[!Info]
> **FirstAuthor**:: Quadri P
> **Title**: Oral Cavity Reconstruction
> **Year**: 2023
> **Citekey**: quadri_OtolaryngolClinNorthAm_2023
> **itemType**: journalArticle
> **Journal**: *Otolaryngol Clin North Am*
> **Volume**: 56
> **Issue**: 4

> [!Abstract]
>
> This summary provides a concise overview of oral cavity reconstruction to optimize functional outcomes in the modern era. Soft tissue and osseous reconstruction options for a wide range of oral cavity sites including lip, oral tongue, floor of mouth, buccal, hard palate, and composite oromandibular resections are reviewed. The appropriate applications of primary closure, secondary intention, skin grafts, and dermal substitute grafts are included. Anatomic considerations, indications, contraindications, and complications of local, regional, and free flaps in oral cavity reconstruction are discussed.
