---
tags:
  - PMID/28692450
citekey: "eskander_CurrOpinOtolaryngolHeadNeckSurg_2017"
dateread: '2026-09-22'
read: false
topic: maxillary_reconstruction
domain: head_neck
relevance: high
arm: review-overview
---
> [!Data]
> **PDF**
> （全文なし）
> **Link**
> https://doi.org/10.1097/MOO.0000000000000396

# 1 AI要約
![[90_attachments/eskander_CurrOpinOtolaryngolHeadNeckSurg_2017/infographic.png]]


## AI要約
### **研究の概要**
- 中顔面の解剖と機能、欠損分類、および medical modeling・デンタルスプリント・歯科リハビリを含む再建手技を幅広く概観したレビュー。

### **先行研究との差異および優位性**
- 「reconstructive ladder を超えて」という題のとおり、階層的手技選択では捉えきれない個別性を強調する。

### **研究手法**
- ナラティブレビュー。

### **結果**
- 中顔面・上顎欠損の分類システムは歴史的に多数存在するが、**大半の欠損は固有であり個別化アプローチが必要**。
- 骨を含む局所有茎皮弁への関心が再燃している。
- 骨皮性前腕皮弁と腓骨皮弁は、骨結合型インプラントの成功率を高めるべく改良されてきた。
- **肩甲骨tip皮弁**が汎用性・良好な骨輪郭の適合・長い血管茎により人気を高めている。
- **デンタルスプリントは medical modeling やカッティングガイドより費用対効果が高い**が、両者は複雑な再建において補完的。
- ⚠️ レビューのため症例数・統計値なし。

### **考察および課題**
- 「分類だけでは皮弁選択は決まらない」という主張は、chen_AnnPlastSurg_2020 のアルゴリズム的アプローチと緊張関係にある。両者を対比するとDiscussionが立体的になる。
- VSPの費用対効果を論じる際に引用できる数少ない総説。xu_FrontBioengBiotechnol_2023 のVSP併用ダブルフラップと対比可能。
- 肩甲骨tip皮弁の台頭を2017年時点で指摘しており、tishko_AtlasOralMaxillofacSurgClinNorthAm_2024 への流れを示す。

### **キーワード**
- #midface_reconstruction, #classification, #scapula_free_flap, #virtual_surgical_planning, #dental_rehabilitation

### **wikilink**
- [[Midface Reconstruction]]
- [[Maxillary Reconstruction]]
- [[Scapula Free Flap]]
- [[Fibula Free Flap]]
- [[Radial Forearm Free Flap]]
- [[Virtual Surgical Planning]]
- [[Dental Rehabilitation]]

# 2 Citation
Eskander A, Kang SY, Teknos TN, Old MO. Advances in midface reconstruction: beyond the reconstructive ladder. Curr Opin Otolaryngol Head Neck Surg. 2017;25(5):422-430.

# 3 Related
>[!Info]
> **FirstAuthor**:: Eskander A
> **Title**: Advances in midface reconstruction: beyond the reconstructive ladder
> **Year**: 2017
> **Citekey**: eskander_CurrOpinOtolaryngolHeadNeckSurg_2017
> **itemType**: journalArticle
> **Journal**: *Curr Opin Otolaryngol Head Neck Surg*
> **Volume**: 25
> **Issue**: 5

> [!Abstract]
>
> This article reviews the anatomy and function of the midface, defect classifications and the breadth of common and novel reconstructive techniques including medical modeling, dental splints, and dental rehabilitation. Historically many classification systems for midface and maxillary defects exist but most defects are unique and an individualized approach is required. There has been an increased interest in using regional pedicled flaps with bone for reconstruction. Free flap reconstruction with the osteocutaneous radial forearm and fibula flap have been recently modified to increase the success of osseointegrated implants. Scapular free flap reconstruction has become increasingly popular because of the versatility, good bony contour match, and long pedicle length with the scapular tip flap. Dental splints are more cost effective than medical modeling and cutting guides but both techniques are complimentary in complex reconstruction.
