---
tags:
  - PMID/20932492
citekey: "brown_LancetOncol_2010"
dateread: '2026-09-22'
read: false
topic: maxillary_reconstruction
domain: head_neck
relevance: core
arm: classification
---
> [!Data]
> **PDF**
> [Brown_2010_maxilla_midface_classification_LancetOncol.pdf](file://C:/Users/user/claude/papers/maxillary_reconstruction/fulltext/Brown_2010_maxilla_midface_classification_LancetOncol.pdf)
> **Link**
> https://doi.org/10.1016/S1470-2045(10)70113-3

# 1 AI要約
![[90_attachments/brown_LancetOncol_2010/infographic.png]]


## AI要約
### **研究の概要**
- 上顎・中顔面欠損に対する新しい分類を提唱したPersonal View（レビュー）。著者らの15年間の経験に基づく。
- 中顔面再建を要する患者の大半は悪性腫瘍切除後であり、多くが術後照射を要するという前提に立つ。

### **先行研究との差異および優位性**
- 欠損の複雑さの増大を軸に据えた分類。垂直方向のclassと水平方向のsubclassを組み合わせる。
- **上顎再建で最も広く引用される分類の原典。** 以後の研究の共通言語となった。

### **研究手法**
- 過去15年の自験例と文献レビューに基づく分類の提唱と、欠損型ごとの再建方針の整理。

### **結果**
- 分類に基づく再建方針:
    - **眼窩付属器に及ばない低位欠損** → 顎義歯（dental obturator）で対応可能なことが多い
    - **より広範な上顎欠損** → 遊離皮弁が必要というコンセンサスがある
    - **眼窩・頬部の支持** → 腸骨稜＋内腹斜筋、肩甲骨tip＋広背筋などの**複合皮弁が、軟部皮弁や非血管柄付き移植より確実**。インプラント支持の歯科・眼窩補綴も可能にする
    - **鼻上顎欠損** → 鼻骨の喪失を補うため骨が必要
    - **眼窩上顎欠損** → 局所皮弁や軟部皮弁でより簡便に対応可能
- ⚠️ レビューのため症例数・統計値なし。

### **考察および課題**
- **本研究のMethodsで欠損分類を定義する際の必読文献。** 群分けの共通言語として引用必須。
- 「骨と筋の複合皮弁 > 軟部皮弁」という主張は rogers_EurArchOtorhinolaryngol_2022 の実践データ（軟部皮弁優位）と**対立する**。2010年と2022年の間に頬骨インプラントが状況を変えた可能性を論じる余地がある。
- 顎義歯という非手術的選択肢を明確に位置づけている点も重要。

### **キーワード**
- #brown_classification, #maxillary_reconstruction, #midface_reconstruction, #obturator, #composite_flap

### **wikilink**
- [[Brown Classification]]
- [[Maxillary Reconstruction]]
- [[Midface Reconstruction]]
- [[Obturator]]
- [[Scapula Free Flap]]
- [[Orbital Floor Reconstruction]]
- [[Dental Rehabilitation]]

# 2 Citation
Brown JS, Shaw RJ. Reconstruction of the maxilla and midface: introducing a new classification. Lancet Oncol. 2010;11(10):1001-8.

# 3 Related
>[!Info]
> **FirstAuthor**:: Brown JS
> **Title**: Reconstruction of the maxilla and midface: introducing a new classification
> **Year**: 2010
> **Citekey**: brown_LancetOncol_2010
> **itemType**: journalArticle
> **Journal**: *Lancet Oncol*
> **Volume**: 11
> **Issue**: 10

> [!Abstract]
>
> Most patients requiring midface reconstruction have had ablative surgery for malignant disease, and most require postoperative radiotherapy. This Personal View is based on a new classification of the midface defect, which emphasises the increasing complexity of the problem. Low defects not involving the orbital adnexae can often be successfully treated with dental obturators. For the more extensive maxillary defects, there is consensus that a free flap is required. Composite flaps of bone and muscle harvested from the iliac crest with internal oblique or the scapula tip with latissimus dorsi can more reliably support the orbit and cheek than soft-tissue free flaps and non-vascularised grafts, and also enable an implant-borne dental or orbital prosthesis. Nasomaxillary defects usually require bone to augment the loss of the nasal bones, but orbitomaxillary cases can be managed more simply with local or soft-tissue free flaps.
