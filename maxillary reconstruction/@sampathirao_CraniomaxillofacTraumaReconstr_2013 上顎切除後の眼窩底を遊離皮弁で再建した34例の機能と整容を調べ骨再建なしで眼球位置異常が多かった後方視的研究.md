---
tags:
  - PMID/24436744
citekey: "sampathirao_CraniomaxillofacTraumaReconstr_2013"
dateread: '2026-09-24'
read: false
topic: maxillary_reconstruction
domain: head_neck
relevance: high
arm: extended-defect
---
> [!Data]
> **PDF**
> [Sampathirao_2013_orbital_floor_reconstruction_free_flaps_after_maxillectomy_CMTR.txt](file://C:/Users/user/claude/papers/maxillary_reconstruction/fulltext/Sampathirao_2013_orbital_floor_reconstruction_free_flaps_after_maxillectomy_CMTR.txt) ※PMC全文テキスト（PDFではない）
> **Link**
> https://doi.org/10.1055/s-0033-1343777

# 1 AI要約


## 要約

上顎の全摘で眼窩底を失っても、眼球を温存できるかどうかは、眼窩底の再建にかかっている。インドの Amrita 医科学研究所のグループは、**眼窩底を含む上顎切除と遊離皮弁再建を受けた34例**を後方視的に解析し、**生存かつ無病で6か月以上追跡できた28例**に機能と整容の横断調査を行った。

再建は**骨再建26例、軟部再建8例**、最も多い皮弁は**遊離腓骨皮弁（14例）**だった。結果は、**視力の正常が86%、眼球運動の正常が92%、整容の満足（good）が19例（68%）**で、**眼球位置の異常が9例**にあった。視力、眼球運動、整容の満足は、骨再建と軟部再建で有意差がなかったが、**骨再建をしなかった群で眼球位置の異常が有意に多かった（p=0.040）**。

著者は**骨性の眼窩再建を支持**しつつ、『検出力があり、群の分布が均等な大規模研究で確かめる必要がある』と留保している。**この留保は適切である。** 骨26例に対して軟部8例と群の大きさが大きく違い、p=0.040は多数の比較のうちの1つである。眼窩内容を摘出した例が軟部再建に多く含まれるかどうかも、抄録から分からない。

## AI要約
### **研究の概要**
- **上顎切除後の眼窩底を遊離皮弁で再建**した症例の機能・整容の成績。
- 34例の後方視的解析＋28例の横断調査（生存・無病・6か月以上）。
- Amrita医科学研究所（インド、頭頸部外科）。

### **先行研究との差異および優位性**
- **眼窩底の再建の成績を、患者自身の機能（視力・眼球運動）と整容の満足**で評価した点。
- 骨再建と軟部再建を、眼球位置の異常で比較している。

### **研究手法**
- 後方視的に、眼窩底を含む上顎切除を悪性腫瘍で受け、遊離皮弁で再建した**連続34例**。
- 生存かつ無病で6か月以上追跡できた**28例**に、機能と整容の**横断調査**。
- 骨再建（26例）と軟部再建（8例）を比較。最多の皮弁は遊離腓骨皮弁（14例）。
- 📌 PMC収載だが、取得できたのは抄録のみ。PDFは未取得。

### **結果**
- **骨再建26例、軟部再建8例。遊離腓骨皮弁が最多（14例）。**
- **視力の正常 86%、眼球運動の正常 92%。**
- **眼球位置の異常 9例。**
- **整容の満足（good）19例（68%）。**
- 視力・眼球運動・整容の満足は骨/軟部で有意差なし。
- **骨再建なしの群で眼球位置の異常が有意に多い（p=0.040）。**

### **考察および課題**
- **眼球を温存した上で、視力86%・眼球運動92%が保たれる**という数字は、遊離皮弁による眼窩底再建が機能を保てることを示す。ただし満足が68%（good）にとどまる点は、整容の限界を示す。
- ⚠️ **『骨再建で眼球位置が保たれる』は、骨26例 vs 軟部8例、p=0.040という弱い根拠である。** 著者自身が大きな研究が要ると留保している。
- **[[@dugast_HeadNeck_2023 Brown III型上顎切除後の眼窩底再建を20研究で整理し遊離骨皮弁と人工物が眼球成績で良好とした系統的レビュー|Dugast S 2023]]（軟部再建で外反・眼球陥凹が多い）や [[@hanasono_PlastReconstrSurg_2013 上顎の遊離皮弁再建246例から口蓋歯槽と眼窩の状態で皮弁を選ぶ包括的アルゴリズムを構築した後方視的研究|Hanasono MM 2013]]（眼窩底は硬性再建が必要）と同じ方向**で、3つの報告が揃う。眼窩底には硬い支えが要るという点で一貫している。
- ⚠️ 機能・整容の調査は生存・無病で6か月以上追跡できた28例のみで、**対象外の6例（34例中）の状態は反映されない**。生存者に偏った評価である。
- [[眼窩底再建]] に整理した。

### **キーワード**
- #orbital_floor, #free_flap, #maxillectomy, #globe_position, #aesthetic_satisfaction

### **wikilink**
- [[眼窩底再建]]
- [[上顎再建]]
- [[遊離腓骨皮弁]]
- [[骨移植 vs 軟部再建]]

# 2 Citation
Sampathirao LMCSR, Thankappan K, Duraisamy S, Hedne N, Sharma M, Mathew J, Iyer S. Orbital floor reconstruction with free flaps after maxillectomy. Craniomaxillofac Trauma Reconstr. 2013;6(2):99-106.

# 3 Related
>[!Info]
> **FirstAuthor**:: Sampathirao LMCSR
> **Title**: Orbital floor reconstruction with free flaps after maxillectomy
> **Year**: 2013
> **Citekey**: sampathirao_CraniomaxillofacTraumaReconstr_2013
> **itemType**: journalArticle
> **Journal**: *Craniomaxillofac Trauma Reconstr*
> **Volume**: 6
> **Issue**: 2

> [!Abstract]
>
> Background The purpose of this study is to evaluate the outcome of orbital floor reconstruction with free flaps after maxillectomy. Methods This was a retrospective analysis of 34 consecutive patients who underwent maxillectomy with orbital floor removal for malignancies, reconstructed with free flaps. A cross-sectional survey to assess the functional and esthetic outcome was done in 28 patients who were alive and disease-free, with a minimum of 6 months of follow-up. Results Twenty-six patients had bony reconstruction, and eight had soft tissue reconstruction. Free fibula flap was the commonest flap used (n = 14). Visual acuity was normal in 86%. Eye movements were normal in 92%. Abnormal globe position resulted in nine patients. Esthetic satisfaction was good in 19 patients (68%). Though there was no statistically significant difference in outcome of visual acuity, eye movement, and patient esthetic satisfaction between patients with bony and soft tissue reconstruction, more patients without bony reconstruction had abnormal globe position (p = 0.040). Conclusion Free tissue transfer has improved the results of orbital floor reconstruction after total maxillectomy, preserving the eye. Good functional and esthetic outcome was achieved. Though our study favors a bony orbital reconstruction, a larger study with adequate power and equal distribution of patients among the groups would be needed to determine this. Free fibula flap remains the commonest choice when a bony reconstruction is contemplated.
