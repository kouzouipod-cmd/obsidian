---
tags:
  - PMID/32390775
citekey: "badhey_SeminPlastSurg_2020"
dateread: '2026-09-24'
read: false
topic: maxillary_reconstruction
domain: head_neck
relevance: core
arm: bone-flap-comparison
title_ja: '口蓋上顎再建における腓骨皮弁と肩甲骨皮弁の利点欠点を比較したレビュー'
title: 'Palatomaxillary Reconstruction: Fibula or Scapula'
first_author: 'Badhey AK'
year: 2020
journal: 'Semin Plast Surg'
design: 'Review'
domain_ja: '頭頸部再建'
topic_ja: '上顎・中顔面再建'
---
> [!Data]
> **PDF**
> [badhey_SeminPlastSurg_2020 PDF](file://C:/Users/user/Zotero/storage/JTUZ6B3A/Badhey_2020_palatomaxillary_fibula_or_scapula_SeminPlastSurg.pdf)
> **Link**
> https://doi.org/10.1055/s-0040-1709431

# 1 AI要約
![[90_attachments/badhey_SeminPlastSurg_2020/infographic.png]]


## AI要約
### **研究の概要**
- 口蓋上顎再建における2大骨皮弁、腓骨皮弁と肩甲骨皮弁の利点と欠点を比較したレビュー。

### **先行研究との差異および優位性**
- 骨皮弁の選択という一点に絞って正面から比較した総説。上顎という部位に限定している点が実用的。

### **研究手法**
- ナラティブレビュー。

### **結果**
- 顎顔面骨格は重要な機能を担う: 咀嚼を支え鼻腔と口腔を隔てる安定した硬口蓋、中顔面輪郭を保つバットレス支持。
- 遊離組織移植は口蓋上顎欠損の再建において標準的手段となった。
- 頭頸部の骨再建の選択肢は多いが、**腓骨と肩甲骨（およびその変法）が中顔面再建で最も多用される2つ**。
- ⚠️ レビューのため症例数・統計値なし。

### **考察および課題**
- **「骨のみシングルフラップ」群を論じる際の中核文献。**
- 重要な含意: 肩甲骨系皮弁は1皮弁で骨＋大量軟部を供給できるため、**ダブルフラップの代替になりうる**。本研究のRQでは、この選択肢を無視すると「ダブルが必要」という結論が成立しなくなる恐れがある。
- tishko_AtlasOralMaxillofacSurgClinNorthAm_2024 と併読すること。

### **キーワード**
- #fibula_free_flap, #scapula_free_flap, #palatomaxillary, #bone_flap, #review

### **wikilink**
- [[Fibula Free Flap]]
- [[Scapula Free Flap]]
- [[Maxillary Reconstruction]]
- [[Bone Flap vs Soft Tissue Flap]]
- [[Free Flap]]

# 2 Citation
Badhey AK, Khan MN. Palatomaxillary Reconstruction: Fibula or Scapula. Semin Plast Surg. 2020;34(2):86-91.

# 3 Related
>[!Info]
> **FirstAuthor**:: Badhey AK
> **Title**: Palatomaxillary Reconstruction: Fibula or Scapula
> **Year**: 2020
> **Citekey**: badhey_SeminPlastSurg_2020
> **itemType**: journalArticle
> **Journal**: *Semin Plast Surg*
> **Volume**: 34
> **Issue**: 2

> [!Abstract]
>
> Palatomaxillary reconstruction presents a unique challenge for the reconstructive surgeon. The maxillofacial skeleton preserves critical aerodigestive functions—it provides a stable hard palate to support mastication and separate the nasal and oral cavities, and buttress support to provide adequate midface contour. Free tissue transfer has become a routine part of the reconstructive ladder in managing palatomaxillary defects. While there is a wide variety of options for bony reconstruction within the head and neck, the fibula and the scapula, and their variations, have become two of the most commonly used options for midface reconstruction. This review will discuss the advantages and disadvantages of both in specific regard to reconstruction of the palatomaxillary area.
