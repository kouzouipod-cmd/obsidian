---
tags:
  - PMID/31732121
citekey: "mavrogenis_Injury_2019"
dateread: '2026-09-24'
read: false
topic: maxillary_reconstruction
domain: head_neck
relevance: medium
arm: review-overview
title_ja: '複合的な口腔下顎欠損に対するマイクロサージャリー再建の骨皮弁と軟部皮弁の使い分けを整理した総説'
title: 'Microsurgical reconstruction of complex oromandibular defects: An update'
first_author: 'Mavrogenis AF'
year: 2019
journal: 'Injury'
design: 'Review'
domain_ja: '頭頸部再建'
topic_ja: '上顎・中顔面再建'
---
> [!Data]
> **PDF**
> [Mavrogenis_2019_oromandibular_microsurgical_update_Injury.pdf](file://C:/Users/user/claude/papers/maxillary_reconstruction/fulltext/Mavrogenis_2019_oromandibular_microsurgical_update_Injury.pdf)
> **Link**
> https://doi.org/10.1016/j.injury.2019.10.061

# 1 AI要約
![[90_attachments/mavrogenis_Injury_2019/infographic.png]]


## AI要約
### **研究の概要**
- 下顎・舌・口底の複合欠損に対するマイクロサージャリー再建の現況を、骨皮弁と軟部皮弁の使い分けを中心に整理した総説。

### **先行研究との差異および優位性**
- 骨と軟部を分けて選択肢を並べ、術前デジタル計画の進歩まで含めて概観する。

### **研究手法**
- ナラティブレビュー。

### **結果**
- 遊離皮弁は下顎・舌・口底再建の gold standard。
- **遊離腓骨皮弁が複合下顎欠損の第一選択**。採取が容易、造形性に優れ、機能転帰が良好なため。舌・口底再建にも用いられる。
- 骨再建の代替: 腓骨皮弁、腸骨稜皮弁。
- 軟部再建の代替: ALT皮弁、前腕皮弁、鼻唇溝島状皮弁。
- コンピュータ支援術前計画により、欠損の仮想シミュレーションと個別化ステレオリソグラフィモデルの作製が可能になった。
- ⚠️ レビューのため症例数・統計値なし。

### **考察および課題**
- 骨は腓骨・腸骨、軟部はALT・前腕という**標準的な役割分担**を確認できる。本研究の群設定の妥当性の裏づけになる。
- 部位が下顎であり上顎ではない。上顎特有の問題（眼窩支持・鼻腔との隔壁・顎義歯という選択肢）は扱われていない。

### **キーワード**
- #oromandibular_reconstruction, #fibula_free_flap, #anterolateral_thigh_flap, #virtual_surgical_planning, #review

### **wikilink**
- [[Fibula Free Flap]]
- [[Anterolateral Thigh Flap]]
- [[Radial Forearm Free Flap]]
- [[Head and Neck Reconstruction]]
- [[Virtual Surgical Planning]]
- [[Microsurgery]]

# 2 Citation
Mavrogenis AF, Igoumenou VG, Ignatiadis I, Mourouzis K, Rallis G, Spyridonos SG. Microsurgical reconstruction of complex oromandibular defects: An update. Injury. 2019;50 Suppl 5:S117-S122.

# 3 Related
>[!Info]
> **FirstAuthor**:: Mavrogenis AF
> **Title**: Microsurgical reconstruction of complex oromandibular defects: An update
> **Year**: 2019
> **Citekey**: mavrogenis_Injury_2019
> **itemType**: journalArticle
> **Journal**: *Injury*
> **Volume**: 50 Suppl 5
> **Issue**: 

> [!Abstract]
>
> Free flaps are the gold standard for reconstruction of the mandible, tongue and floor of the mouth. Free fibular flaps are the most preferable option for reconstruction of complex mandibular defects, as well as for tongue and mouth floor reconstruction, since they are harvested easily, present excellent sculptability and good functional outcomes. Alternative options for bone reconstruction include the fibular and iliac crest free flap, and for soft tissue reconstruction include the anterolateral thigh, the radial forearm free flap, and the nasolabial island flap. Advances in computerized preoperative planning have allowed virtual simulation of the defect and fabrication of an individualized stereolithic mandibular model.
