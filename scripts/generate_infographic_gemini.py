...
- **ただしグラフに描いてよいのは、AbstractまたはAI要約に明記されている数値だけ**（下の「数値の扱い」を厳守）
...
この図は医師が診療の判断に使う。**数値を創作することは、誤りの中で最も重大である。**

6. **AbstractまたはAI要約に明記されていない数値は、図中のどこにも書かない。**
   軸の目盛り、棒の高さ、点の位置、割合、症例数のすべてが対象である。
7. **散布図・回帰直線・箱ひげ図・エラーバーなど、個々の症例のデータが要る図は描かない。**
   論文の抄録に個票データは載らないため、描けば必ず創作になる。
   相関係数やp値は、図形にせず**数値のテキストとして**示す。
8. 群間比較の棒グラフは、**両群の実数または実際の割合が明記されている場合にのみ**描く。
   数値が片方しか分からないときは、棒グラフにせず数値を並べて書く。
9. 数値が乏しい論文（症例報告・ナラティブレビュー・質的研究など）では、
   **無理にグラフを作らず**、要点を短い文と図解（フロー図・アイコン・対比表）で示す。
   グラフが1つも無い図になっても構わない。
10. 軸ラベルの単位と値は、抄録の記載とそろえる。
    例: 平均容積が115cm³なら、軸の範囲を0-50などにしない。
