---
tags:
  - PMID/
citekey: "hidaka_J.Surg.Oncol._"
dateread: '2026-01-29'
read: false
---
> [!Data]
> **PDF**.
> **Link**
> https://doi.org/10.1002/jso.70267

# 1 AI要約
![[90_attachments/hidaka_J.Surg.Oncol._/infographic.png]]

## AI要約
### **研究の概要**
- 進行口腔癌患者における下顎微小血管再建術後の早期術後合併症をComprehensive Complication Index (CCI)を用いて評価。血管柄付骨移植(VBG)、非骨性皮弁併用下顎再建プレート(MRP)、軟組織皮弁(STF)の3つの再建手技を比較。

### **先行研究との差異および優位性**
- 従来の合併症評価は単一指標に依存していたが、本研究はClavien-Dindo分類に基づくCCIを用いることで、合併症の重症度を包括的かつ定量的に評価。3つの再建手技の比較により、適切な手技選択に関する客観的指標を提供。

### **研究手法**
- 研究デザイン：後向き研究
- 対象：101例の患者
- 主要評価項目：早期術後合併症（≤30日）のCCI値
- 副次評価項目：手技別の合併症発生率
- 統計手法：多変量線形回帰分析

### **結果**
- VBGは若年で健康な患者に実施される傾向。3つの手技間でCCI値に有意差を認めず。多変量解析では糖尿病などの非手術因子が合併症リスクにより大きく影響することが確認された。

### **考察および課題**
- 適切な患者選択により早期合併症リスクを最小化可能。VBGの若年患者への適用とMRPの技術改良により、STFと同等の合併症負荷を達成できる可能性。Limitation：単施設研究、後向きデザイン。今後の課題：多施設での前向き研究による検証。

### **キーワード**
- #mandibular_reconstruction #microvascular_surgery #CCI #oral_cancer #complication_assessment

### **wikilink**
- [[下顎再建]]
- [[微小血管外科]]
- [[合併症評価]]
- [[口腔癌]]
- [[血管柄付骨移植]]

# 2 Citation
Hidaka, Takeaki and Miyamoto, Shimpei and Fukunaga, Yutaka and Oshima, Azusa and Shinozaki, Takeshi and Matsuura, Kazuto and Higashino, Takuya. Early Complications in Mandibular Reconstruction: Analysis Using Comprehensive Complication Index. Journal of Surgical Oncology. Unknown;n/a.

# 3 Related
>[!Info]
> **FirstAuthor**:: Hidaka, Takeaki
> **Title**: Early Complications in Mandibular Reconstruction: Analysis Using Comprehensive Complication Index
> **Year**: Unknown
> **Citekey**: hidaka_J.Surg.Oncol._
> **itemType**: journalArticle
> **Journal**: *Journal of Surgical Oncology*
> **Volume**: n/a
> **Issue**: 
> [!Abstract]
>
> Background Early postoperative complications after microvascular mandibular reconstruction impact outcomes in patients with advanced oral cancer. This study evaluates three reconstruction techniques: vascularized bone graft (VBG), mandibular reconstruction plate (MRP) with a non-osseous flap, and soft tissue flap (STF), using the Comprehensive Complication Index (CCI). Methods In this retrospective study of 101 patients, early postoperative complications (≤ 30 days) were assessed using the CCI, calculated from Clavien-Dindo classification grades. Differences among the three techniques were analyzed using multivariable linear regression. Results VBG was performed in younger, healthier patients. No significant differences in CCI were found among the three techniques. Multivariable analysis confirmed that non-surgical factors, such as diabetes, had greater influence on complication risks. Conclusions Appropriate patient selection minimizes early complication risks. VBG in younger patients and technical refinements in MRP may yield complication burdens comparable to STF. Trial Registration: This study was registered in the UMIN Clinical Trials Registry as UMIN000056584 (http://www.umin.ac.jp/ctr).