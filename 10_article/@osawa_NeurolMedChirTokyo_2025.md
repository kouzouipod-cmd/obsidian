---
tags:
  - PMID/41183915
citekey: "osawa_NeurolMedChirTokyo_2025"
dateread: '2026-01-29'
read: false
topic: legacy_10article
domain: craniofacial
title: 'One-Stage Surgery for Malignant Skull and Scalp Tumor: Navigation-guided Tumor Resection and Cranioplasty Using Custom-made Implants: Technical Note'
first_author: 'OSAWA'
year: 2025
journal: 'Neurologia medico-chirurgica'
domain_ja: '頭蓋顎顔面'
topic_ja: '旧ノート（Zotero Integration）'
---
> [!Data]
> **PDF**.
> **Link**
> https://doi.org/10.2176/jns-nmc.2025-0167

# 1 AI要約
![[90_attachments/osawa_NeurolMedChirTokyo_2025/infographic.png]]

## AI要約
### **研究の概要**
- 悪性頭蓋底・頭皮腫瘍に対する一期的手術（腫瘍切除とカスタムメイドインプラントによる頭蓋形成術）の技術的方法論を報告。ナビゲーションガイド下腫瘍切除とカスタムメイドインプラントを用いた一期的再建により、患者負担軽減と美容的結果の改善を実現。

### **先行研究との差異および優位性**
- 従来の頭蓋底腫瘍手術では二期的手術が一般的だったが、本研究はインプラント設計データをナビゲーションシステムに統合することで一期的手術を可能にした。正確な開頭ラインの決定という技術的課題を解決し、手術精度と安全性を向上させた。

### **研究手法**
- 研究デザイン：技術報告（Technical Note）
- 対象：転移性頭蓋骨腫瘍2例、頭蓋骨浸潤を伴う原発性頭皮肉腫2例
- 手法：インプラント設計データと術前CT画像の融合、ナビゲーションシステムによる開頭ライン決定
- 評価項目：全摘出率、美容的結果、術後合併症

### **結果**
- 全例で総摘出を達成。2例で腫瘍進行により予定切除縁を拡大したが、骨セメントまたはチタンプレートで補填し良好な美容的結果を得た。インプラントと頭蓋骨の適合は満足すべき結果で、術後合併症は認めず。

### **考察および課題**
- 本手技により正確で効率的かつ安全な一期的手術が可能となり、精密な骨切りと最適なインプラント適合を実現。患者負担の軽減、合併症の最小化、美容的結果の改善に寄与。Limitation：症例数の制限、長期追跡の必要性。今後の課題：手技の標準化と適応拡大。

### **キーワード**
- #skull_tumor #cranioplasty #custom_implant #navigation_surgery #one_stage_surgery

### **wikilink**
- [[頭蓋底腫瘍]]
- [[頭蓋形成術]]
- [[カスタムインプラント]]
- [[ナビゲーション手術]]
- [[一期的手術]]

# 2 Citation
OSAWA, Sho and OHNO, Makoto and YANAGISAWA, Shunsuke and MIYAKITA, Yasuji and TAKAHASHI, Masamichi and FUJITA, Shohei and TSUCHIYA, Takahiro and IWATA, Shintaro and OGURA, Koichi and ARIKAWA, Masaki and AKAZAWA, Satoshi and NARITA, Yoshitaka. One-Stage Surgery for Malignant Skull and Scalp Tumor: Navigation-guided Tumor Resection and Cranioplasty Using Custom-made Implants: Technical Note. Neurologia medico-chirurgica. 2025;65:638--644.

# 3 Related
>[!Info]
> **FirstAuthor**:: OSAWA, Sho
> **Title**: One-Stage Surgery for Malignant Skull and Scalp Tumor: Navigation-guided Tumor Resection and Cranioplasty Using Custom-made Implants: Technical Note
> **Year**: 2025
> **Citekey**: osawa_NeurolMedChirTokyo_2025
> **itemType**: journalArticle
> **Journal**: *Neurologia medico-chirurgica*
> **Volume**: 65
> **Issue**: 
> [!Abstract]
>
> One-stage tumor resection and cranioplasty for skull tumors using custom-made implants are beneficial for reducing patient burden, minimizing complications, and improving cosmetic outcomes. However, accurately determining the craniotomy line remains challenging. This technical note describes a method for integrating implant design data into a navigation system to facilitate accurate 1-stage surgery for skull and scalp tumors. Merged images of the implant design data and preoperative computed tomography scans were created. The craniotomy line was determined using a navigation system with merged images. After tumor resection, cranioplasty was performed using a custom-made implant. We applied this technique in 2 patients with metastatic skull tumors and 2 with primary scalp sarcomas with skull invasion. Gross total resection was achieved in all patients. The resection area was expanded beyond the preplanned margins in 2 cases because of tumor progression; the bone defects were filled or covered with bone cement or titanium plates, with positive cosmetic results. The alignment between the implants and skull was satisfactory, and no postoperative complications were observed. This technique enables accurate, efficient, and safe 1-stage surgery by ensuring precise osteotomy and optimal implant fitting in skull and scalp tumors.